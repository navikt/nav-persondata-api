...
class PensjonsgivendeInntektClient(
    ...
    fun hentPensjonsgivendeInntekt(
        ...
        val requestBody =
            SigrunPensjongivendeInntektRequest(
        ...
        return runCatching {
                        ...
                        .exchangeToMono { response ->
                            when {
                                response.statusCode().is2xxSuccessful -> {
                                    response
                                        .bodyToMono(SigrunPensjonsgivendeInntektResponse::class.java)
                                        .map {
                                            println(it)
                                            PensjonsgivendeInntektDataResultat(
                                                data = it,
                                ...
                                response.statusCode().value() == 404 -> {
                                    Mono.just(
                                        PensjonsgivendeInntektDataResultat(
                                            data =
                                                SigrunPensjonsgivendeInntektResponse(
                                                    inntektsaar = inntektsaar.toString(),
                                                    emptyList(),
                                                ),
                                            statusCode = 404,
            ...
            return PensjonsgivendeInntektDataResultat(
                data = null,
                statusCode = 500,
                errorMessage = error.rootCause().message,
            )
        })
    }
