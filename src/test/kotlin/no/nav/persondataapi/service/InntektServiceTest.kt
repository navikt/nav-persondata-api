...
class InntektServiceTest {
    ...
    fun `skal maskere data når saksbehandler ikke har tilgang`() =
        runBlocking {
            ...
            every { inntektClient.hentInntekter(any(), any()) } returns
                InntektDataResultat(
    ...
    @Test
    fun `skal returnere PersonIkkeFunnet når InntektClient returnerer 404`() =
        runBlocking {
            val brukertilgangService = mockk<BrukertilgangService>()
            val inntektClient = mockk<InntektClient>()
            val eregClient = mockk<EregClient>()
    ...
    @Test
    fun `skal returnere IngenTilgang når InntektClient returnerer 403`() =
        runBlocking {
            val brukertilgangService = mockk<BrukertilgangService>()
            val inntektClient = mockk<InntektClient>()
            val eregClient = mockk<EregClient>()
    ...
    @Test
    fun `skal returnere FeilIBaksystem når InntektClient returnerer 500`() =
        runBlocking {
            val brukertilgangService = mockk<BrukertilgangService>()
            val inntektClient = mockk<InntektClient>()
            val eregClient = mockk<EregClient>()
    ...
    @Test
    fun `skal returnere FeilIBaksystem når InntektClient returnerer annen feilkode`() =
        runBlocking {
            val brukertilgangService = mockk<BrukertilgangService>()
            val inntektClient = mockk<InntektClient>()
            val eregClient = mockk<EregClient>()
    ...
    @Test
    fun `skal returnere tom liste når person ikke har inntekter`() =
        runBlocking {
            val brukertilgangService = mockk<BrukertilgangService>()
            val inntektClient = mockk<InntektClient>()
            val eregClient = mockk<EregClient>()
    ...
    @Test
    fun `skal mappe lønnsinntekt med organisasjonsinformasjon fra Ereg`() =
        runBlocking {
            val brukertilgangService = mockk<BrukertilgangService>()
            val inntektClient = mockk<InntektClient>()
            val eregClient = mockk<EregClient>()
    ...
    @Test
    fun `skal velge nyeste versjon av inntektsinformasjon`() =
        runBlocking {
            val brukertilgangService = mockk<BrukertilgangService>()
            val inntektClient = mockk<InntektClient>()
            val eregClient = mockk<EregClient>()
    ...
    @Test
    fun `skal velge nyeste versjon av inntektsinformasjon også der inntekt er borte i nyeste versjon`() =
        runBlocking {
            val brukertilgangService = mockk<BrukertilgangService>()
            val inntektClient = mockk<InntektClient>()
            val eregClient = mockk<EregClient>()
    ...
    @Test
    fun `skal håndtere flere historikkperioder for samme person`() =
        runBlocking {
            val brukertilgangService = mockk<BrukertilgangService>()
            val inntektClient = mockk<InntektClient>()
            val eregClient = mockk<EregClient>()
