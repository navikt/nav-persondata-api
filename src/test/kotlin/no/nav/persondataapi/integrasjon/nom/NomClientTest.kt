...
class NomClientTest {
    ...
    @Test
    fun `hentSaksbehandlerTilhørighet skal mappe data ved suksess`() =
        runBlocking {
    ...
    @Test
    fun `hentSaksbehandlerTilhørighet skal returnere 404 ved not_found feil`() =
        runBlocking {
            mockkConstructor(GraphQLWebClient::class)
            val nomClient = NomClient(tokenService, metrics, nomUrl)
    ...
    @Test
    fun `hentSaksbehandlerTilhørighet skal returnere 500 ved ukjent feil i respons`() =
        runBlocking {
            mockkConstructor(GraphQLWebClient::class)
            val nomClient = NomClient(tokenService, metrics, nomUrl)
    ...
    @Test
    fun `hentSaksbehandlerTilhørighet skal returnere 404 når ressurs er null`() =
        runBlocking {
            mockkConstructor(GraphQLWebClient::class)
            val nomClient = NomClient(tokenService, metrics, nomUrl)
    ...
    @Test
    fun `hentSaksbehandlerTilhørighet skal returnere 504 ved timeout`() =
        runBlocking {
            mockkConstructor(GraphQLWebClient::class)
            val nomClient = NomClient(tokenService, metrics, nomUrl)

            every { tokenService.getServiceToken(SCOPE.NOM_SCOPE) } returns "token"
            coEvery { anyConstructed<GraphQLWebClient>().execute(any<RessursQuery>(), any()) } throws
    ...
    @Test
    fun `hentSaksbehandlerTilhørighet skal returnere 500 ved uventet exception`() =
        runBlocking {
            mockkConstructor(GraphQLWebClient::class)
            val nomClient = NomClient(tokenService, metrics, nomUrl)
